...
const page=()=>{
    ...
    useEffect(()=>{
        ...
        if(query.get('success')){
            toast.success('order placed! You will receive an email conformation.');
            deleteAllItems()
        }
        if(query.get('canceled')){
            toast.error('order canceled--continue to shp around and checkout when you are ready.')
        }
    },[])
                                    ...
                                    <div key={item.id} className="flex justify-between border rounded-md p-4 my-2 bg-white hover:shadow-lg">
                                        ...
                                        <div className="flex items-center gap-5">
                                            <div className="flex items-center gap-3">
                                                <button disabled={item.quantity==1}className="p-1 rounded-md text-orage-500 hover:bg-orange-500 hover:text-white disabled:cursor-not-allowed" 
                                                ...
                                                <button className="p-1 rounded-md text-orange-500 hover:bg-orange-500 hover:text-white disable:cursor-not-allowed" 
                                            ...
                                            <button className="text-orange-500 hover:text-red-600" onClick={()=>deleteById(item.id)}>
