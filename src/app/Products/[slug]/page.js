...
export async function generateMetadata({params:{slug}}){
    ...
    return{
        title:`Patil Product | ${product.name}`
    }
}
        ...
        <div className="m-2 px-20">
            <div className="flex justify-around items-center flex-wrap">
                <div className="w-80 h-80">
                    <Image src={product.images[0]} alt={product.name} className="w-80 h-80" height={80} width={80} />
                </div>
                <div className="flex=1 max-w-md border rounded-md shadow-lg p-6 bg-white">
                    <h2 className="text-3xl font-semibold">{product.name}</h2>
                    <div className="flex pt-2 gap-2">
                        <CheckIcon className="text-line-500 w-5 h-5"/>
